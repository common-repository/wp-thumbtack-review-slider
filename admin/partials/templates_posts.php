 ...
 */
	...
	if (isset($_POST['wpthumbtack_submittemplatebtn'])){
		//verify nonce wp_nonce_field( 'wpthumbtack_save_template');
		check_admin_referer( 'wpthumbtack_save_template');

		//get form submission values and then save or update
		$t_id = htmlentities($_POST['edittid']);
		$title = htmlentities($_POST['wpthumbtack_template_title']);
		$template_type = htmlentities($_POST['wpthumbtack_template_type']);
		$style = htmlentities($_POST['wpthumbtack_template_style']);
		$display_num = htmlentities($_POST['wpthumbtack_t_display_num']);
		$display_num_rows = htmlentities($_POST['wpthumbtack_t_display_num_rows']);
		$display_order = htmlentities($_POST['wpthumbtack_t_display_order']);
		$hide_no_text = htmlentities($_POST['wpthumbtack_t_hidenotext']);
		$template_css = htmlentities($_POST['wpthumbtack_template_css']);
		
		$createslider = htmlentities($_POST['wpthumbtack_t_createslider']);
		$numslides = htmlentities($_POST['wpthumbtack_t_numslides']);
		...
		$templatemiscarray['showstars']=htmlentities($_POST['wpthumbtack_template_misc_showstars']);
		$templatemiscarray['showdate']=htmlentities($_POST['wpthumbtack_template_misc_showdate']);
		$templatemiscarray['bgcolor1']=htmlentities($_POST['wpthumbtack_template_misc_bgcolor1']);
		$templatemiscarray['bgcolor2']=htmlentities($_POST['wpthumbtack_template_misc_bgcolor2']);
		$templatemiscarray['tcolor1']=htmlentities($_POST['wpthumbtack_template_misc_tcolor1']);
		$templatemiscarray['tcolor2']=htmlentities($_POST['wpthumbtack_template_misc_tcolor2']);
		$templatemiscarray['tcolor3']=htmlentities($_POST['wpthumbtack_template_misc_tcolor3']);
		$templatemiscarray['bradius']=htmlentities($_POST['wpthumbtack_template_misc_bradius']);
		...
		if($t_id==""){
			...
			if($updatetempquery>0){
				$dbmsg = '<div id="setting-error-wpthumbtack_message" class="updated settings-error notice is-dismissible">'.__('<p><strong>Template Updated!</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button>', 'wp-fb-reviews').'</div>';
			}
		}
	...
	$reviews_table_name = $wpdb->prefix . 'wpthumbtack_reviews';
	$reviewtotalcount = $wpdb->get_var( 'SELECT COUNT(*) FROM '.$reviews_table_name );
	if($reviewtotalcount<1){
		$dbmsg = $dbmsg . '<div id="setting-error-wpthumbtack_message" class="updated settings-error notice is-dismissible">'.__('<p><strong>No reviews found. Please visit the <a href="?page=wp_thumbtack-get_thumbtack">Get Thumbtack Reviews</a> page to retrieve reviews from Thumbtack.</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button>', 'wp-fb-reviews').'</div>';
	}
...
<div class="wrap wp_thumbtack-settings">
	<h1><img src="<?php echo plugin_dir_url( __FILE__ ) . 'logo.png'; ?>"></h1>
<?php 
include("tabmenu.php");
?>
<div class="wpthumbtack_margin10">
	<a id="wpthumbtack_helpicon_posts" class="wpthumbtack_btnicononly button dashicons-before dashicons-editor-help"></a>
	<a id="wpthumbtack_addnewtemplate" class="button dashicons-before dashicons-plus-alt"><?php _e('Add New Reviews Template', 'wp-fb-reviews'); ?></a>
</div>
...
echo $dbmsg;
	...
	$urltrimmed = remove_query_arg( array('taction', 'id') );
		...
		$html .= '<tr id="'.$currentform->id.'">
				<th scope="col" class="wpthumbtack_upgrade_needed manage-column">'.$currentform->id.'</th>
				<th scope="col" class="wpthumbtack_upgrade_needed manage-column"><b>'.$currentform->title.'</b></th>
				<th scope="col" class="wpthumbtack_upgrade_needed manage-column"><b>'.$currentform->template_type.'</b></th>
				<th scope="col" class="wpthumbtack_upgrade_needed manage-column">'.date("F j, Y",$currentform->created_time_stamp) .'</th>
				<th scope="col" class="manage-column" templateid="'.$currentform->id.'" templatetype="'.$currentform->template_type.'"><a href="'.$url_tempeditbtn.'" class="button button-primary dashicons-before dashicons-admin-generic">'.__('Edit', 'wp-fb-reviews').'</a>, <a href="'.$url_tempdelbtn.'" class="button button-secondary dashicons-before dashicons-trash">'.__('Delete', 'wp-fb-reviews').'</a>, <a class="wpthumbtack_displayshortcode button button-secondary dashicons-before dashicons-visibility">'.__('Shortcode', 'wp-fb-reviews').'</a></th>
			</tr>';
	}	
		$html .= '</tbody></table>';
 ...
 echo $html;			
?>
<div class="wpthumbtack_margin10" id="wpthumbtack_new_template">
<form name="newtemplateform" id="newtemplateform" action="?page=wp_thumbtack-templates_posts" method="post" onsubmit="return validateForm()">
	<table class="wpthumbtack_margin10 form-table ">
		<tbody>
			<tr class="wpthumbtack_row">
				<th scope="row">
					<?php _e('Template Title:', 'wp-fb-reviews'); ?>
				</th>
				<td>
					<input id="wpthumbtack_template_title" data-custom="custom" type="text" name="wpthumbtack_template_title" placeholder="" value="<?php echo $currenttemplate->title; ?>" required>
					<p class="description">
			...
			<tr class="wpthumbtack_row">
				...
				<td><div id="divtemplatestyles">
					...
					<input type="radio" name="wpthumbtack_template_type" id="wpthumbtack_template_type1-radio" value="post" checked="checked">
					<label for="wpthumbtack_template_type1-radio"><?php _e('Post or Page', 'wp-fb-reviews'); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

					<input type="radio" name="wpthumbtack_template_type" id="wpthumbtack_template_type2-radio" value="widget" <?php if($currenttemplate->template_type== "widget"){echo 'checked="checked"';}?>>
					<label for="wpthumbtack_template_type2-radio"><?php _e('Widget Area', 'wp-fb-reviews'); ?></label>
					</div>
					<p class="description">
...
<tr class="wpthumbtack_row">
				...
				<td>
					<div class="w3_wprs-row">
						  <div class="w3_wprs-col s6">
							...
							<div class="w3_wprs-col s6">
								<div class="wprevpre_temp_label_row">
									<select name="wpthumbtack_template_style" id="wpthumbtack_template_style">
				...
				?>
								<div class="wprevpre_temp_label_row">
									<select name="wpthumbtack_template_misc_showstars" id="wpthumbtack_template_misc_showstars">
								...
								<div class="wprevpre_temp_label_row">
									<select name="wpthumbtack_template_misc_showdate" id="wpthumbtack_template_misc_showdate">
								...
								<div class="wprevpre_temp_label_row">
									<input id="wpthumbtack_template_misc_bradius" type="number" min="0" name="wpthumbtack_template_misc_bradius" placeholder="" value="<?php echo $template_misc_array['bradius']; ?>" style="width: 4em">
								</div>
								<div class="wprevpre_temp_label_row">
									<input type="text" data-alpha="true" value="<?php echo $template_misc_array['bgcolor1']; ?>" name="wpthumbtack_template_misc_bgcolor1" id="wpthumbtack_template_misc_bgcolor1" class="my-color-field" />
								</div>
								<div class="wprevpre_temp_label_row wprevpre_bgcolor2">
									<input type="text" data-alpha="true" value="<?php echo $template_misc_array['bgcolor2']; ?>" name="wpthumbtack_template_misc_bgcolor2" id="wpthumbtack_template_misc_bgcolor2" class="my-color-field" />
								</div>
								<div class="wprevpre_temp_label_row">
									<input type="text" value="<?php echo $template_misc_array['tcolor1']; ?>" name="wpthumbtack_template_misc_tcolor1" id="wpthumbtack_template_misc_tcolor1" class="my-color-field" />
								</div>
								<div class="wprevpre_temp_label_row">
									<input type="text" value="<?php echo $template_misc_array['tcolor2']; ?>" name="wpthumbtack_template_misc_tcolor2" id="wpthumbtack_template_misc_tcolor2" class="my-color-field" />
								</div>
								<div class="wprevpre_temp_label_row wprevpre_tcolor3">
									<input type="text" value="<?php echo $template_misc_array['tcolor3']; ?>" name="wpthumbtack_template_misc_tcolor3" id="wpthumbtack_template_misc_tcolor3" class="my-color-field" />
								</div>
								
								
								<a id="wpthumbtack_pre_resetbtn" class="button"><?php _e('Reset Colors', 'wp-review-slider-pro'); ?></a>
							</div>
						  </div>
						  <div class="w3_wprs-col s6" id="wpthumbtack_template_preview">

						  </div>
					</div>
					<p class="description">
					<?php _e('More styles available in <a href="?page=wp_thumbtack-get_pro">Pro Version</a> of plugin!', 'wp-fb-reviews'); ?></p>
				</td>
			</tr>
			<tr class="wpthumbtack_row">
				<th scope="row">
				...
				<td>
					<textarea name="wpthumbtack_template_css" id="wpthumbtack_template_css" cols="50" rows="4"><?php echo $currenttemplate->template_css; ?></textarea>
					<p class="description">
					<?php _e('Enter custom CSS code to change the look of the template when being displayed. ex: </br>.wpthumbtack_t1_outer_div {
						background: #e4e4e4;
					}', 'wp-fb-reviews'); ?></p>
				</td>
			</tr>			
			...
			<tr class="wpthumbtack_row">
				...
				<td><div class="divtemplatestyles">
					<label for="wpthumbtack_t_display_num"><?php _e('How many per a row?', 'wp-fb-reviews'); ?></label>
					<select name="wpthumbtack_t_display_num" id="wpthumbtack_t_display_num">
					...
					<label for="wpthumbtack_t_display_num_rows"><?php _e('How many total rows?', 'wp-fb-reviews'); ?></label>
					<input id="wpthumbtack_t_display_num_rows" type="number" name="wpthumbtack_t_display_num_rows" placeholder="" value="<?php if($currenttemplate->display_num_rows>0){echo $currenttemplate->display_num_rows;} else {echo "1";}?>">
			...
			<tr class="wpthumbtack_row">
				...
				<td>
					<select name="wpthumbtack_t_display_order" id="wpthumbtack_t_display_order">
			...
			<tr class="wpthumbtack_row">
				...
				<td>
					<select name="wpthumbtack_t_hidenotext" id="wpthumbtack_t_hidenotext">
			...
			<tr class="wpthumbtack_row">
				...
				<td>
					<div class="divtemplatestyles">
						<label for="wpthumbtack_t_createslider"><?php _e('Display reviews in slider?', 'wp-fb-reviews'); ?></label>
						<select name="wpthumbtack_t_createslider" id="wpthumbtack_t_createslider">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<label for="wpthumbtack_t_display_num_rows"><?php _e('How many total slides?', 'wp-fb-reviews'); ?></label>
						<select name="wpthumbtack_t_numslides" id="wpthumbtack_t_numslides">
			...
			<tr>
				<td class="notice updated " colspan="2" style="border-left: 4px solid #d6d6d6;">
					<p><strong><?php _e('Upgrade to the Pro Version of this plugin to access more super cool settings! <a href="#TB_inline?tempnum=1&inlineId=mythickboxid&height=650&width=1030" class="thickbox">(Screenshot)</a> Get the Pro Version <a href="?page=wp_thumbtack-get_pro">here</a>!', 'wp-fb-reviews'); ?></strong></p>
				</td>
			</tr>
	...
	<?php 
	//security nonce
	wp_nonce_field( 'wpthumbtack_save_template');
	?>
	<input type="hidden" name="edittid" id="edittid"  value="<?php echo $currenttemplate->id; ?>">
	<input type="submit" name="wpthumbtack_submittemplatebtn" id="wpthumbtack_submittemplatebtn" class="button button-primary" value="<?php _e('Save Template', 'wp-fb-reviews'); ?>">
	<a id="wpthumbtack_addnewtemplate_cancel" class="button button-secondary"><?php _e('Cancel', 'wp-fb-reviews'); ?></a>
	</form>
</div>
<div class=""><p>Do you like this plugin? If so please take a moment to leave us a review <a href="https://wordpress.org/plugins/wp-thumbtack-review-slider/" target="blank">here!</a> If it's missing something then please contact us <a href="http://ljapps.com/contact/" target="blank">here</a>. Thanks!</p></div>
	<div id="popup_review_list" class="popup-wrapper wpthumbtack_hide">
