(function( $ ) {
	...
	$(function(){
		...
		//help button clicked
		$( "#wpthumbtack_helpicon" ).click(function() {
		...
		//remove all button
		$( "#wpthumbtack_removeallbtn" ).click(function() {
		  openpopup("Are you sure?", '<p>This will delete all reviews in your Wordpress database including the ones you manually entered. It Does NOT affect your reviews on Facebook.</p>', '<a class="button dashicons-before dashicons-no" href="?page=wp_thumbtack-reviews&opt=delall">Remove</a>');
		});	

		//upgrade to pro
		$( ".wpthumbtack_upgrade_needed" ).click(function() {
		  openpopup("Upgrade Needed", '<p>Please upgrade to the Pro Version of this Plugin to access this feature.</p>', '<a class="button dashicons-before  dashicons-cart" href="?page=wp_thumbtack-get_pro">Upgrade Here</a>');
		});		
		...
		$( "#wpthumbtack_addnewreviewbtn" ).click(function() {
		  jQuery("#wpthumbtack_new_review").show("slow");
		});	
		$( "#wpthumbtack_addnewreview_cancel" ).click(function() {
		  jQuery("#wpthumbtack_new_review").hide("slow");
		  //reload page without taction and tid
		  setTimeout(function(){ 
			window.location.href = "?page=wp_thumbtack-reviews"; 
		  }, 500);
		...
		$('#upload_avatar_button').click(function() {
			tb_show('Upload Reviewer Avatar', 'media-upload.php?referer=wp_thumbtack-reviews&type=image&TB_iframe=true&post_id=0', false);
			return false;
		});
		...
		window.send_to_editor = function(html) {
			...
			$('#wpthumbtack_nr_avatar_url').val(image_url);
		...
		$("#newreviewform").submit(function(){ 
			  ...
			  if ($('input[name=wpthumbtack_nr_rating]:checked').length) {
			...
			if(jQuery( "#wpthumbtack_nr_name").val()==""){
				alert("Please enter a name.");
				$( "#wpthumbtack_nr_name" ).focus();
				return false;
			} else {
				return true;
			}
		...
		$("#review_list").on("click", ".reveditbtn", function (event) {
			...
			$("#wpthumbtack_new_review").show("slow");
			//find values from rowobject and fill in edit form wpthumbtack_nr_rating, wpthumbtack_nr_text, wpthumbtack_nr_name, wpthumbtack_nr_avatar_url, wpthumbtack_nr_date
			$("#editrid").val(rid);
			$("#wpthumbtack_nr_name").val(name);
			$("#wpthumbtack_nr_avatar_url").val(wprev_row_userpic);
			//for radio
			$("#wpthumbtack_nr_rating").val(wprev_row_rating);
			$("#wpthumbtack_nr_date").val(wprev_row_created_time);
			
			$("#wpthumbtack_nr_text").val(wprev_row_review_text);
		...
		function sendtoajax(rid,whattodo,rowobject){
			var senddata = {
					action: 'wpthumbtack_hide_review',	//required
		...
		//for pagination bar-----------------------------------
		$("#wpthumbtack_review_list_pagination_bar").on("click", "span", function (event) {
		...
		function sendtoajaxreview(pageclicked,sortbyval,sortd,selrevs){
			...
			$( "#review_list" ).html("");
			$( "#wpthumbtack_review_list_pagination_bar" ).html("");
			var senddata = {
				...
				var reviewtext = '';
					...
					$.each(object, function(index) {
						...
						if(object[index].reviewer_name){
							...
							if(object[index].type=="Facebook"){
								...
								editdellink ='';
							} else if(object[index].type=="Thumbtack"){
					...
					if(numpages>1){
						...
						$( "#wpthumbtack_review_list_pagination_bar" ).html(pagebarhtml);

					if(reviewtotalcount==0){
						$("#wpthumbtack_review_list_pagination_bar").hide();
					} else {

						$("#wpthumbtack_review_list_pagination_bar").show();
					}
