 ...
 * @subpackage WP_Thumbtack_Review/public/partials
 */
...
for ($x = 0; $x < count($rowarray); $x++) {
	if(	$currentform[0]->template_type=="widget"){
		?>
		<div class="wpthumbtack_t1_outer_div_widget w3_wprs-row-padding-small">
		<?php
		} else {
		?>
		<div class="wpthumbtack_t1_outer_div w3_wprs-row-padding">
		<?php
	}
	//loop 
	foreach ( $rowarray[$x] as $review ) 
	{
		...
		if($userpic==""){
			...
			$userpichtml ='<img src="'.$userpic.'" alt="thumb" class="wpthumbtack_t1_IMG_4" />';
		}
		
		//star number 
		if($review->type=="Thumbtack"){
			//find business url
			$options = get_option('wpthumbtack_thumbtack_settings');
			$burl = $options['thumbtack_business_url'];
			if($burl==""){
				$burl="https://www.thumbtack.com";
			}
			$starfile = "stars_".$review->rating."_yellow.png";
			$logo = '<a href="'.$burl.'" target="_blank" rel="nofollow"><img src="'.$imgs_url.'thumbtack_outline.png" alt="" class="wpthumbtack_t1_thumbtack_logo"></a>';
		} 
		...
		if($review->review_text !=""){
			...
			//add thumbtack fix for ..More replace with link to pageid
	...
	?>
		<div class="wpthumbtack_t1_DIV_1<?php if(	$currentform[0]->template_type=="widget"){echo ' marginb10';}?> w3_wprs-col l<?php echo $perrow; ?>">
			<div class="wpthumbtack_t1_DIV_2 wprev_preview_bg1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> wprev_preview_bradius_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
				<p class="wpthumbtack_t1_P_3 wprev_preview_tcolor1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
					<span class="wpthumbtack_star_imgs_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><img src="<?php echo $imgs_url."".$starfile; ?>" alt="" class="wpthumbtack_t1_star_img_file">&nbsp;&nbsp;</span><?php echo stripslashes($reviewtext); ?>
				</p>
				<?php echo $logo; ?>
			</div><span class="wpthumbtack_t1_A_8"><?php echo $userpichtml; ?></span> <span class="wpthumbtack_t1_SPAN_5 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo stripslashes($review->reviewer_name); ?><br/><span class="wprev_showdate_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $tempdate; ?></span> </span>
		</div>
	<?php
	}
	//end loop
