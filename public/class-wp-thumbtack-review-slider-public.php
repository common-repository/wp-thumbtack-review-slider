 ...
 * @subpackage WP_Thumbtack_Review/public
 */
 ...
 */
class WP_Thumbtack_Review_Public {
	...
	public function enqueue_styles() {
		...
		foreach ( $templatearray as $template ){
			if(isset($template->style)){
				if($template->style=="1" || $template->style=="2" || $template->style=="3" || $template->style=="4" || $template->style=="5" || $template->style=="6" || $template->style=="7" || $template->style=="8" || $template->style=="9" || $template->style=="10"){
					wp_register_style( 'wp-thumbtack-review-slider-public_template'.$template->style, plugin_dir_url( __FILE__ ) . 'css/wprev-public_template'.$template->style.'.css', array(), $this->version, 'all' );
					wp_enqueue_style( 'wp-thumbtack-review-slider-public_template'.$template->style );
				}
			}
		}

		wp_register_style( 'wpthumbtack_w3', plugin_dir_url( __FILE__ ) . 'css/wpthumbtack_w3.css', array(), $this->version, 'all' );
		...
		wp_enqueue_style( 'wpthumbtack_w3' );
	 ...
	 * @since    1.0.0
	 */
	public function shortcode_wpthumbtack_usetemplate() {
	
				add_shortcode( 'wpthumbtack_usetemplate', array($this,'wpthumbtack_usetemplate_func') );
	}	 
	public function wpthumbtack_usetemplate_func( $atts, $content = null ) {
		//get attributes
		    $a = shortcode_atts( array(
				...
				ob_start();
				include plugin_dir_path( __FILE__ ) . '/partials/wp-thumbtack-review-slider-public-display.php';
				return ob_get_clean();
	}
}
